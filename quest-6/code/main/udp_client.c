...
// Defines for udp communication
#define HOST_IP_ADDR "192.168.1.111" //ip address of ESP_OK
#define PORT 3030
...
int slowdown = 0; // Yellowlight Flag
int manual = 0; // Manual driving Flag
int redlight = 0; // Red light Flag
int turn = 0; // Right turn Flag
...
// Task for remotely stablishing a udp connection with raspberry pi
...
// Function that sends information to the display and makes it display the split times
...
// Function for sending split times to the Node.js
void sendsplittime() {
...
// Task for reading the transmissions from the beacons
  ...
  data = (uint8_t *) malloc(256);
    ...
    while (1) {
        ...
        //if (len >= 1) {
          ...
          // assigning value to flags depending on the data read
          while (i < 4) {
            i++;
            if(data[i] == 0x1B) {
              if (i != 3) {
...
// Initialization for timer
    ...
    int count = 0;
      while (1) {
          ...
          // Run PID and adjust for driving straight every 100 ms (when timer is triggered)
          if (dt_complete == 1) {
              count++;
              if (count == 10) {
                  setspeedcontroller();
