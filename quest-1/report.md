...
5. Investigative question response: How can you synchronize multiple ESP clocks with each other? You can synchronize multiple ESP clocks using FreeRTOS. In our project, we are creating one task for the alphanumeric display, one task for the seconds servo to keep track of seconds passing, and one task for minutes servo to keep track of minutes passing
